...
(defn think [grid crt-player depth max-chance]
  (if (zero? depth)
    ...
    (loop [[cell & rest-cells] (filter #(= -1 (get-in grid %)) 
      ...
      (cond
        (nil? cell) ret
        (and max-chance (>= cs max-chance))
        [0x7fffffff nil]
        :else
        (let [newgrid (assoc-in grid cell crt-player)
              enm-ret (think newgrid (switch-player crt-player) (dec depth) (- cs))
...
(defn ^:export play 
  ...
  ([grid crt-player depth] 
   (let [vec-grid (vec (map vec (vec grid)))
     ...
     (apply ret->js (think vec-grid crt-player depth nil)))))
